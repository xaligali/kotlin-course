package lessons.lesson04

// PART 1
val v1: Int = 42
val v2: Long = 98765432123456789L
val v3: Float = 23.45f
val v4: Double = 0.123456789
val v5: String = "Kotlin & Java"

// FALSE написано с заглавных букв. В Kotlin используется false
// val v6: Boolean = FALSE

val v7: Char = 'c'
val v8: Int = 500
val v9: Long = 4294967296L
val v10: Float = 18.0f
val v11: Double = -0.001
val v12: String = "OpenAI"
val v13: String = "true"
val v14: List<Int> = listOf(3, 14)
val v15: Char = '9'
val v16: Int = 2048
val v17: Long = 10000000000L
val v18: Set<String> = setOf("OpenAI", "Quantum Computing")
val v19: Float = 5.75f

// в обратных кавычках нельзя записать число с точкой как идентификатор
// val v20: Double = `1.414`

val v21: String = "Artificial Intelligence"

// Элементы массива имеют разные типы, поэтому тип массива — Array<Any>
val v22: Array<Any> = arrayOf('x', "A")

val v23: String = "Android Studio"
val v24: Char = '@'
val v25: Int = 1024
val v26: Long = 1234567890123L
val v27: Float = 10.01f
val v28: Double = -273.15
val v29: String = "SpaceX"

// FALSE написано с заглавных букв, в Kotlin используется false
// val v30: Boolean = FALSE

val v31: Double = 0.007

// использованы кавычки “ ” вместо обычных кавычек " "
// val v32: String = “🤯”

val v33: Map<String, Int> = mapOf("true" to 2, "false" to 34)

// использованы кавычки ‘ ’, а также внутри Char находится несколько символов
// val v34: Char = ‘65535’

val v35: Long = 72057594037927935L
val v36: Float = 2.71828f
val v37: Double = 101.0101
val v38: String = "Quantum Computing"
val v39: Map<Int, String> = mapOf(2 to "true", 34 to "false")
val v40: Char = 'x'
val v41: Int = 314
val v42: Long = 123456789123456789L
val v43: Float = 6.626f

// TRUE написано с заглавных букв, в Kotlin используется true
// val v44: Boolean = TRUE


// PART 2
val type1: Char = 'A'
val type2: Int = 3
val type3: List<Double> = listOf(4.50, 7.99, 2.30)
val type4: Double = 12.345678901
val type5: Boolean = true
val type6: Int = 5
val type7: Long = 123456789L
val type8: Long = 9876543210L
val type9: Double = 15.275
val type10: Map<String, Double> = mapOf(
    "OpenAI" to 1000000.0,
    "Claudi" to 250000.50
)
val answer11: String = "починилось само"
val answer12: List<String> = listOf(
    "ненужная тема №1",
    "ненужная тема №2",
    "ненужная тема №3"
)